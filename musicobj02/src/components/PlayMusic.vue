<template>
    <div class="playMusic">
        ...
        <div v-else class="playContent"  @click="isLyric =! isLyric">
            <!-- active控制小白条的落下与抬起 -->
            <!-- 因为obvious默认是true 与音乐格式不符 所以要取反!obvious -->
            <img class="needle" :class="{active: !obvious}" src="@/assets/images/needle-ip6.png" alt="">
        ...
        <div class="playFooter">
            ...
            <svg class="icon" aria-hidden="true" v-if="obvious" @click="play">
...
export default {
    name: "playmusic",
    props:['obvious','play','playDetail'],
    data() {
        return {
            isLyric: false  //控制歌词与唱片的显示
        }
    },
    computed: {
    ...
    methods: {
        tabMusic(num) {
            // console.log(num)
            let index = this.playCurrentIndex+num  //切换后的下标
            if( index<0 )
            {
                index = this.playlist.length-1;
                // console.log(this.playlist.length)
                // this.setPlayIndex(index);  //修改当前
